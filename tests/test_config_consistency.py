...
ENV_KEY_RE = re.compile(r'^([A-Z_]+)=', re.MULTILINE)
CONFIG_BLOCK_RE = re.compile(r'#+ .*?Configuration.*?\n.*?```.*?\.env\n(.*?)\n\s*```', re.DOTALL | re.IGNORECASE)
README_KEY_RE = re.compile(r'([A-Z_]+)=')
...
def test_config_consistency():
    ...
    env_keys = set(ENV_KEY_RE.findall(env_content))
